...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden lg:flex flex-col h-auto gap-4 items-center rounded-2xl bg-gray-50 dark:bg-neutral-900 px-4 py-3",
        className
      )}
    >
      {items.map((item) => (
        ...
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              exit={{ opacity: 0, x: 10 }}
              className="px-2 py-0.5 whitespace-pre rounded-md bg-gray-100 border dark:bg-neutral-800 dark:border-neutral-900 dark:text-white border-gray-200 text-neutral-700 absolute left-24 w-fit text-xs"
            >
              {title}
            </motion.div>
          )}
