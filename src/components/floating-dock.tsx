  ...
  return (
    <div className={cn("relative block lg:hidden", className)}>
            ...
            {items.map((item, idx) => (
              ...
              >
                <Link
                  href={item.href}
                  key={item.title}
                  className="h-14 w-14 rounded-full bg-neutral-900 flex items-center justify-center"
                >
      ...
      <button
        ...
        className="h-14 w-14 rounded-full bg-neutral-800 flex items-center justify-center"
      >
...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden lg:flex flex-col h-auto gap-4 items-center rounded-2xl bg-neutral-900 px-4 py-3",
        className
      )}
    >
      {items.map((item) => (
        ...
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              exit={{ opacity: 0, x: 10 }}
              className="px-2 py-0.5 whitespace-pre rounded-md bg-neutral-800 border-neutral-900 text-white absolute left-24 w-fit text-xs"
            >
              {title}
            </motion.div>
          )}
